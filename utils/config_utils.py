...
def auto_name(configs_dict, config_diffs):
    """Helper function for automatically naming datasets based on configs."""
    new_config_dict = {}
    for seed, config_list in configs_dict.items():
        ...
        for config, config_diff in zip(config_list, config_diffs):
            ...
            name = name.replace("'", "")
            config.dataset_name = name + '_s' + str(seed)
    ...
    if default_name:
        for seed, config_list in configs_dict.items():
            for i, config in enumerate(config_list):
                config.dataset = str(i).zfill(6) + '_s' + str(seed)  # default name
                config.save_path = os.path.join(ROOT_DIR, 'experiments',
